...
class client_flow(object):
    ...
    def initializer(self, exe_path, num_send, bytes_each, my_port, server_port, color):
        ...
        # Initialize plot
        line, = self.plot.plot([], [], label=self.name, color=color)  # Create an empty line and return it
        self.line = line
        self.plot.legend()
        self.initialized = True

    def start_flow(self, start_time): # todo, time arg to set flow starting time
    ...
    def data_point(self, pack_rtt, pack_end_time, pack_num): # todo. instead of redrawing. Only draw in the order of pack num. If pack num not in order, add it to a list, or hash with key = expected index
    ...
    def draw_line(self):
        ...
        # Adjust the axes to accommodate new data
